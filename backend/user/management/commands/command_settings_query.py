...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for currency_dict in CURRENCY:
            ...
            if not CommandCurrency.objects.filter(pk=currency_dict.get('pk')):
                currency_model = CommandCurrency(**currency_dict)
                currency_model.save()
                self.stdout.write(f'Currency "{currency}" added.')
            else:
        ...
        for pagination_dict in PAGINATION:
            ...
            if not CommandPagination.objects.filter(pk=pagination_dict.get('pk')):
                paginate_by_model = CommandPagination(**pagination_dict)
                paginate_by_model.save()
                self.stdout.write(f'Pagination "{paginate_by}" added.')
            else:
